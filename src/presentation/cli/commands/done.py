...
def done_command(ctx, task_ids):
    ...
    for task_id in task_ids:
        try:
            ...
            console.print(f"  [yellow]⚠[/yellow] The following child tasks are not completed:")
            for child in e.incomplete_children:
                status_color = "blue" if child.status == TaskStatus.IN_PROGRESS else "yellow"
                console.print(f"    - Task {child.id}: {child.name} ([{status_color}]{child.status.value}[/{status_color}])")
            console.print(f"  [dim]Complete all child tasks first, then try again.[/dim]")
            error_count += 1
            if len(task_ids) > 1:
                console.print()
