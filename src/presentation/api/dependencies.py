...
from presentation.api.context import ApiContext
...
def get_api_context() -> ApiContext:
    ...
    if _api_context is None:
        raise RuntimeError("API context not initialized. Call initialize_api_context() first.")
    return _api_context
...
LifecycleControllerDep = Annotated[TaskLifecycleController, Depends(get_lifecycle_controller)]
RelationshipControllerDep = Annotated[
    TaskRelationshipController, Depends(get_relationship_controller)
]
AnalyticsControllerDep = Annotated[TaskAnalyticsController, Depends(get_analytics_controller)]
