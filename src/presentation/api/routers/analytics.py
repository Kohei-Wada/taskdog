...
from presentation.api.models.requests import OptimizeScheduleRequest
        ...
        response = StatisticsResponse(
            ...
            estimation=(
                EstimationStatistics(
                    ...
                    average_deviation_percentage=result.estimation_stats.accuracy_rate * 100,
...
async def get_gantt_chart(
    ...
    start_date: Annotated[str | None, Query(description="Chart start date (ISO format)")] = None,
    end_date: Annotated[str | None, Query(description="Chart end date (ISO format)")] = None,
    ...
    task_daily_hours = {
        task_id: {date_obj.isoformat(): hours for date_obj, hours in daily_hours.items()}
...
def run_optimization(
    controller, algorithm: str, start_date: datetime, max_hours_per_day: float, force_override: bool
) -> None:
