...
class TestConfigManager:
    ...
    @pytest.mark.parametrize(
        "toml_content,expected_planned_start_time",
        [
            (None, time(9, 30)),
            (
                """
[time]
default_planned_start_time = "10:00"
""",
                time(10, 0),
            ),
...
""",
    ...
    def test_config_loading_scenarios(
        ...
        assert config.time.default_planned_start_time == expected_planned_start_time
    ...
    def test_config_dataclasses_are_frozen(self):
        ...
        with pytest.raises((FrozenInstanceError, AttributeError)):
            config.time.default_planned_start_time = time(10, 0)
...
class TestConfigManagerTimeFormat:
    """Test cases for time format support in ConfigManager."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                """
[time]
default_planned_start_time = "09:30"
""",
                9,
                30,
            ),
            (
                """
[time]
default_planned_start_time = "10:45"
""",
                10,
                45,
            ),
            (
                """
[time]
default_planned_start_time = "11:00"
""",
                11,
                0,
            ),
    ...
    def test_time_config_loading(self, toml_content, expected_hour, expected_minute):
        ...
        assert config.time.default_planned_start_time.hour == expected_hour
        assert config.time.default_planned_start_time.minute == expected_minute

    def test_env_var_time_format(self):
        """Test environment variable with time format."""
        env_vars = {
            "TASKDOG_TIME_DEFAULT_PLANNED_START_TIME": "10:30",
        }

        with patch.dict(os.environ, env_vars, clear=False):
            config = ConfigManager.load(Path("/nonexistent/config.toml"))

        assert config.time.default_planned_start_time == time(10, 30)
...
class TestConfigManagerEnvVars:
    ...
    def test_env_vars_override_defaults(self):
        ...
        env_vars = {
            "TASKDOG_TIME_DEFAULT_PLANNED_START_TIME": "10:00",
        }

        with patch.dict(os.environ, env_vars, clear=False):
            config = ConfigManager.load(Path("/nonexistent/config.toml"))

        assert config.time.default_planned_start_time == time(10, 0)
...
"""
        env_vars = {
            "TASKDOG_TIME_DEFAULT_PLANNED_START_TIME": "08:30",
        }
        ...
        try:
            ...
            assert config.time.default_planned_start_time == time(8, 30)
            # TOML value should be used when no env var is set
            assert config.time.default_deadline_time == time(17, 0)
        finally:
            config_path.unlink()
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "TASKDOG_TIME_DEFAULT_PLANNED_START_TIME",
                "10:00",
                "time",
            ...
            (
                "TASKDOG_TIME_DEFAULT_DEADLINE_TIME",
                "20:00",
                "time",
    ...
    def test_no_env_vars_uses_defaults(self):
        ...
        # Should use defaults
        assert config.time.default_planned_start_time == time(9, 30)
        assert config.time.default_deadline_time == time(18, 30)
    ...
    @pytest.mark.parametrize(
        ...
        ids=["planned_start_time", "deadline_time"],
    )
